...
**Memory bank discipline (mandatory):** If `memory-bank/` or its files are missing, create them before any code actions using this order and templates: `projectbrief.md` → `activeContext.md` → `SESSION.md` → `README.md`. Log sessions as `YYYY-MM-DD — vX.Y.Z` (append-only). Maintain `memory-bank/master-plan.md` for multi-step efforts.
